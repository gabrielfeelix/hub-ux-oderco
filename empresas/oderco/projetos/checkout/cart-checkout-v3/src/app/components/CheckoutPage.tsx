...
═══════════════════════════════════════════════════════════ */
type FlowStepKey = 'frete' | 'pagamento' | 'detalhes' | 'review';

function MainStepIndicator({ flowStep, accentColor }: { flowStep: FlowStepKey; accentColor?: string }) {
  const accent = accentColor || 'var(--primary)';

  /* Ordem linear global — as 2 notas avançam juntas pelos mesmos passos. */
  const order: FlowStepKey[] = ['frete', 'pagamento', 'detalhes', 'review'];
  const idx = order.indexOf(flowStep);

  const steps = [
    { key: 'carrinho',  label: 'Carrinho',  done: true,        active: false },
    { key: 'frete',     label: 'Frete',     done: idx > 0,     active: flowStep === 'frete' },
    { key: 'pagamento', label: 'Pagamento', done: idx > 1,     active: flowStep === 'pagamento' },
    { key: 'detalhes',  label: 'Detalhes',  done: idx > 2,     active: flowStep === 'detalhes' },
    { key: 'revisao',   label: 'Revisão',   done: false,       active: flowStep === 'review' },
  ];
...
export function CheckoutPage() {
  ...
  /* Novo: ?nfs=PR,ES — conjunto de notas selecionadas no carrinho (checkbox ML-style).
     Legado: ?nf=PR — uma nota só (variantes V2/V3 antigas). */
  const nfsParam = searchParams.get('nfs');
  const scopedFiliais: ('PR' | 'ES')[] | null = nfsParam
    ? (nfsParam.split(',').filter(x => x === 'PR' || x === 'ES') as ('PR' | 'ES')[])
    : (nfParam === 'PR' || nfParam === 'ES' ? [nfParam] : null);
  /* scopedNf mantido só p/ a lógica legada de split de crédito (single-NF). */
  const scopedNf: ('PR' | 'ES') | null = scopedFiliais && scopedFiliais.length === 1 ? scopedFiliais[0] : null;

  /* ── Global flow step — PARALELO: as 2 notas avançam JUNTAS pelos mesmos passos.
       Não há mais "nota por vez"; cada passo renderiza os blocos de TODAS as filiais. ── */
  type FlowStep = 'frete' | 'pagamento' | 'detalhes' | 'review';
  const [flowStep, setFlowStep] = useState<FlowStep>('frete');
  ...
  /* Consultor removido — sem opção de contato. Mantido como false p/ não renderizar avisos. */
  const wantConsultor = false;

  /* SubStep type mantido só p/ assinaturas de validação por-nota. O passo do fluxo
     agora é `flowStep` (global, paralelo). */
  ...
  const activeFiliaisInCart = useMemo<('PR' | 'ES')[]>(() => {
    const present: ('PR' | 'ES')[] = [];
    if (itemsPR.length > 0) present.push('PR');
    if (itemsES.length > 0) present.push('ES');
    /* Escopo da seleção do carrinho (?nfs=...): só as notas escolhidas E presentes. */
    if (scopedFiliais) {
      const scoped = present.filter(f => scopedFiliais.includes(f));
      if (scoped.length > 0) return scoped;
    }
    return present;
  }, [itemsPR.length, itemsES.length, scopedFiliais?.join(',')]);
  ...
  /* ── Scroll to top on every flow step change (after React commits the new render) ── */
  useEffect(() => {
  ...
  /* Passo só avança quando TODAS as notas satisfazem a validação daquele passo. */
  const canProceedFlowStep = (step: FlowStep): boolean => {
    if (step === 'review' || step === 'detalhes') return true;
    return activeFiliaisInCart.every(f => canProceedSubStep(f, step));
  };

  /* ── Navigation PARALELA — flowStep global avança as 2 notas juntas ── */
  const FLOW_ORDER: FlowStep[] = ['frete', 'pagamento', 'detalhes', 'review'];

  const handleSaveAndContinue = () => {
    const idx = FLOW_ORDER.indexOf(flowStep);
    if (idx < FLOW_ORDER.length - 1) setFlowStep(FLOW_ORDER[idx + 1]);
  };

  const handleBack = () => {
    const idx = FLOW_ORDER.indexOf(flowStep);
    if (idx <= 0) { navigate('/carrinho'); return; }
    setFlowStep(FLOW_ORDER[idx - 1]);
  };
  ...
  /* ═══════════════════════════════════════════════════════════
     RENDER: NF SIDEBAR
  ═══════════════════════════════════════════════════════════ */
  /* Card de resumo de UMA nota — usado empilhado p/ cada filial no sidebar paralelo. */
  const renderNfSummaryCard = (filial: 'PR' | 'ES') => {
  ...
  /* ═══════════════════════════════════════════════════════════
     SIDEBAR COMBINADO (PARALELO) — resumo de cada nota empilhado
     + card "Total Geral" somando as 2 + checklist de progresso.
  ═══════════════════════════════════════════════════════════ */
  const renderCombinedSidebar = () => {
    const filiais = activeFiliaisInCart;
    const combinedSubtotal = filiais.reduce((s, f) => s + (f === 'PR' ? subtotalPR : subtotalES), 0);
    const combinedShipping = filiais.reduce((s, f) => s + getShippingPriceForNf(f), 0);
    const combinedCredit   = filiais.reduce((s, f) => s + (f === 'PR' ? creditAmountPR : creditAmountES), 0);
    const combinedTotal    = filiais.reduce((s, f) => s + (f === 'PR' ? totalPRAfterCredit : totalESAfterCredit), 0);
    ...
    return (
      <div className="w-full lg:w-[380px] xl:w-[400px] shrink-0 order-first lg:order-last">
        <div className="lg:sticky lg:top-5 flex flex-col gap-3">
          ...
          {/* Resumo por nota */}
          {filiais.map(f => <div key={f}>{renderNfSummaryCard(f)}</div>)}

          {/* Total Geral — só faz sentido quando há 2+ notas */}
          {filiais.length > 1 && (
            <div className="rounded-xl p-4 flex flex-col gap-2.5" style={{ background: 'var(--foreground)', boxShadow: 'var(--elevation-sm)' }}>
              <span className="block" style={{ fontSize: 'var(--text-2xs)', fontWeight: 'var(--font-weight-bold)', color: 'rgba(255,255,255,0.6)', fontFamily: 'var(--font-red-hat-display)', textTransform: 'uppercase', letterSpacing: '1px' }}>
                Total Geral · {filiais.length} notas fiscais
              </span>
              <div className="flex items-center justify-between">
                <span style={{ fontSize: 'var(--text-xs)', color: 'rgba(255,255,255,0.7)', fontFamily: 'var(--font-red-hat-display)' }}>Produtos</span>
                <span style={{ fontSize: 'var(--text-xs)', color: 'var(--primary-foreground)', fontFamily: 'var(--font-red-hat-display)', fontVariantNumeric: 'tabular-nums' }}>{formatCurrency(combinedSubtotal)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span style={{ fontSize: 'var(--text-xs)', color: 'rgba(255,255,255,0.7)', fontFamily: 'var(--font-red-hat-display)' }}>Frete</span>
                <span style={{ fontSize: 'var(--text-xs)', color: combinedShipping === 0 ? 'var(--primary)' : 'var(--primary-foreground)', fontWeight: combinedShipping === 0 ? 'var(--font-weight-bold)' : 'var(--font-weight-normal)', fontFamily: 'var(--font-red-hat-display)', fontVariantNumeric: 'tabular-nums' }}>
                  {combinedShipping === 0 ? 'GRÁTIS' : formatCurrency(combinedShipping)}
                </span>
              </div>
              {combinedCredit > 0 && (
                <div className="flex items-center justify-between">
                  <span style={{ fontSize: 'var(--text-xs)', color: 'rgba(255,255,255,0.7)', fontFamily: 'var(--font-red-hat-display)' }}>Crédito aplicado</span>
                  <span style={{ fontSize: 'var(--text-xs)', color: 'var(--primary)', fontWeight: 'var(--font-weight-bold)', fontFamily: 'var(--font-red-hat-display)', fontVariantNumeric: 'tabular-nums' }}>− {formatCurrency(combinedCredit)}</span>
                </div>
              )}
              <div className="h-px w-full" style={{ background: 'rgba(255,255,255,0.14)' }} />
              <div className="flex items-center justify-between">
                <span style={{ fontSize: 'var(--text-sm)', fontWeight: 'var(--font-weight-bold)', color: 'var(--primary-foreground)', fontFamily: 'var(--font-red-hat-display)' }}>Total a pagar</span>
                <span style={{ fontSize: 'var(--text-lg)', fontWeight: 'var(--font-weight-bold)', color: 'var(--primary-foreground)', fontFamily: 'var(--font-red-hat-display)', fontVariantNumeric: 'tabular-nums' }}>{formatCurrency(combinedTotal)}</span>
              </div>
            </div>
          )}
    ...
    <div style={{ background: 'var(--background)', fontFamily: 'var(--font-red-hat-display)' }}>
      ...
      <div className="relative overflow-hidden" style={{ background: 'var(--foreground)' }}>
        ...
        <div className="relative max-w-[1280px] mx-auto px-4 md:px-6 pt-7 pb-10 text-center">
          <MainStepIndicator flowStep="review" />
            ...
            {activeFiliaisInCart.map((f) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex flex-col gap-4"
                >
                  {/* ═══ HEADER DA NOTA — divide claramente PR de ES ═══ */}
                  {hasMultipleOrigins && (
                    <div className="flex items-center gap-3 px-1 pt-1">
                      <span className="rounded-lg flex items-center justify-center shrink-0"
                        style={{ width: 34, height: 34, background: ns.color, color: 'var(--primary-foreground)', fontSize: 13, fontWeight: 'var(--font-weight-bold)', fontFamily: 'var(--font-red-hat-display)', letterSpacing: '0.4px' }}>
                        {f}
                      </span>
                      <div className="flex-1 min-w-0">
                        <span className="block" style={{ fontSize: 'var(--text-base)', fontWeight: 'var(--font-weight-bold)', color: ns.color, fontFamily: 'var(--font-red-hat-display)', lineHeight: 1.1 }}>
                          Nota Fiscal {f} · {f === 'PR' ? 'Paraná' : 'Espírito Santo'}
                        </span>
                        <span className="block mt-0.5" style={{ fontSize: 'var(--text-2xs)', color: 'var(--muted-foreground)', fontFamily: 'var(--font-red-hat-display)' }}>
                          {nfQty} {nfQty === 1 ? 'item' : 'itens'} · {formatCurrency(nfTotal)}
                        </span>
                      </div>
                    </div>
                  )}

                  {/* ═══ CARD ÚNICO — Entrega/Retirada + Pagamento (seções separadas por divisor) ═══ */}
                  <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--card)', border: '1px solid var(--muted)', boxShadow: 'var(--elevation-sm)' }}>
                    ...
                    <div className="flex items-center gap-2.5 px-5 py-3.5" style={{ borderBottom: '1px solid var(--muted)' }}>
                      <span className="rounded-lg flex items-center justify-center shrink-0"
                        style={{ width: 32, height: 32, background: ns.color, color: 'var(--primary-foreground)' }}>
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    {/* ── divisor entre Entrega/Retirada e Pagamento ── */}
  ...
  /* ── Configuração PARALELA — render dos blocos das 2 notas no mesmo passo ── */
  const accent = 'var(--primary)';

  /* Cabeçalho de seção por nota — divide visualmente PR de ES dentro do passo. */
  const renderNfSectionHeader = (f: 'PR' | 'ES') => {
    const fStyle = NF_STYLE[f];
    const fItems = f === 'PR' ? itemsPR : itemsES;
    const fSubtotal = f === 'PR' ? subtotalPR : subtotalES;
    const qty = fItems.reduce((s, i) => s + i.quantity, 0);
    return (
      <div className="flex items-center gap-2.5 pt-1">
        <span className="rounded flex items-center justify-center shrink-0"
          style={{ width: 22, height: 22, background: fStyle.color, color: 'var(--primary-foreground)', fontSize: 10, fontWeight: 'var(--font-weight-bold)', fontFamily: 'var(--font-red-hat-display)', letterSpacing: '0.3px' }}>
          {f}
        </span>
        <span style={{ fontSize: 'var(--text-base)', fontWeight: 'var(--font-weight-bold)', color: 'var(--foreground)', fontFamily: 'var(--font-red-hat-display)' }}>
          Nota {f === 'PR' ? 'Paraná' : 'Espírito Santo'}
        </span>
        <span style={{ fontSize: 'var(--text-xs)', color: 'var(--muted-foreground)', fontFamily: 'var(--font-red-hat-display)' }}>
          · {qty} {qty === 1 ? 'item' : 'itens'} · {formatCurrency(fSubtotal)}
        </span>
        <span className="flex-1 h-px ml-1" style={{ background: 'var(--muted)' }} />
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--background)', fontFamily: 'var(--font-red-hat-display)' }}>
      ...
      {/* ── Page Header — band navy + identidade das 2 notas ── */}
      <div className="relative overflow-hidden" style={{ background: 'var(--foreground)' }}>
        <div className="absolute inset-0 pointer-events-none opacity-[0.07]"
          style={{ backgroundImage: 'radial-gradient(circle at center, var(--primary-foreground) 1px, transparent 1.5px)', backgroundSize: '22px 22px' }} />
        <div className="relative max-w-[1280px] mx-auto px-4 md:px-6 pt-6 pb-7 text-center">
          <MainStepIndicator flowStep={flowStep} accentColor={accent} />
          <div className="mt-4 inline-flex items-center gap-2 rounded-md px-2.5 py-1"
            style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.14)' }}>
            {activeFiliaisInCart.map(f => (
              <span key={f}
                className="rounded flex items-center justify-center shrink-0"
                style={{ width: 18, height: 18, background: NF_STYLE[f].color, color: 'var(--primary-foreground)', fontSize: 9, fontWeight: 'var(--font-weight-bold)', fontFamily: 'var(--font-red-hat-display)', letterSpacing: '0.3px' }}>
                {f}
              </span>
            ))}
            <span style={{ fontSize: 'var(--text-xs)', fontWeight: 'var(--font-weight-bold)', color: 'var(--primary-foreground)', fontFamily: 'var(--font-red-hat-display)', letterSpacing: '0.2px' }}>
              {hasMultipleOrigins ? `${activeFiliaisInCart.length} notas fiscais` : `Nota ${activeFiliaisInCart[0]}`}
            </span>
      ...
      <div className="max-w-[1280px] mx-auto flex flex-col lg:flex-row gap-8 pt-5 pb-16 px-4 md:px-6">
        ...
        {/* Left column — passo atual, blocos das 2 notas empilhados */}
        <div className="flex-1 min-w-0 flex flex-col gap-5">

          {/* ── Guiding header — frase amigável muda por passo ── */}
          <div className="mb-1">
            <span className="block" style={{ fontSize: 'var(--text-2xs)', fontWeight: 'var(--font-weight-bold)', color: 'var(--primary)', fontFamily: 'var(--font-red-hat-display)', letterSpacing: '1.5px', textTransform: 'uppercase' }}>
              {flowStep === 'frete' ? 'PASSO 1 DE 3' : flowStep === 'pagamento' ? 'PASSO 2 DE 3' : 'PASSO 3 DE 3'}
            </span>
            <h3 className="m-0 mt-1.5" style={{ fontSize: 'clamp(22px, 3vw, 28px)', fontWeight: 'var(--font-weight-bold)', color: 'var(--foreground)', fontFamily: 'var(--font-red-hat-display)', lineHeight: 1.15 }}>
              {flowStep === 'frete' && <>Vamos começar pela entrega.<br /><span style={{ color: 'var(--primary)' }}>Como você quer receber cada nota?</span></>}
              {flowStep === 'pagamento' && <>Agora, vamos ao pagamento.<br /><span style={{ color: 'var(--primary)' }}>Como prefere pagar cada nota?</span></>}
              {flowStep === 'detalhes' && <>Quase lá!<br /><span style={{ color: 'var(--primary)' }}>Ajuste os últimos detalhes.</span></>}
            </h3>
            <p className="mt-2 mb-0" style={{ fontSize: 'var(--text-sm)', color: 'var(--muted-foreground)', fontFamily: 'var(--font-red-hat-display)', lineHeight: 1.5 }}>
              {flowStep === 'frete' && (hasMultipleOrigins ? 'Cada nota fiscal escolhe sua própria logística — CIF, FOB ou retirada. Elas seguem juntas no checkout.' : 'Escolha a logística: entrega CIF, transportadora FOB ou retirada na filial.')}
              {flowStep === 'pagamento' && (hasMultipleOrigins ? 'Cada nota pode ter forma e condição de pagamento independentes. Seu crédito é compartilhado entre elas.' : 'Use seu crédito disponível, escolha o método e a condição.')}
              {flowStep === 'detalhes' && 'Adicione observações do pedido ou da nota fiscal, se precisar. Campo opcional.'}
            </p>
          </div>

          {/* ── PASSO 1: Frete — bloco por nota ── */}
          {flowStep === 'frete' && (
            <AnimatePresence mode="wait">
              <motion.div key="step-frete" className="flex flex-col gap-6"
                initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}
                transition={{ duration: 0.22, ease: 'easeOut' }}>
                {activeFiliaisInCart.map(f => (
                  <div key={f} className="flex flex-col gap-3">
                    {hasMultipleOrigins && renderNfSectionHeader(f)}
                    {renderShippingBlock(f)}
                  </div>
                ))}
              </motion.div>
            </AnimatePresence>
          ...
          {/* ── PASSO 2: Pagamento — crédito compartilhado + forma por nota ── */}
          {flowStep === 'pagamento' && (
            <AnimatePresence mode="wait">
              <motion.div key="step-pagamento" className="flex flex-col gap-6"
                initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}
                transition={{ duration: 0.22, ease: 'easeOut' }}>
                {renderCreditBlock()}
                {activeFiliaisInCart.map(f => (
          ...
          {/* ── PASSO 3: Detalhes (observações por nota) ── */}
          {flowStep === 'detalhes' && (
            <>
          ...
          {/* ── Footer navigation — passo global das 2 notas ── */}
          {(() => {
            const stepCanProceed = canProceedFlowStep(flowStep);
            const backLabel = flowStep === 'frete' ? '← Voltar ao Carrinho'
              : flowStep === 'pagamento' ? '← Voltar ao Frete'
              : '← Voltar ao Pagamento';
            const nextLabel = flowStep === 'frete' ? 'Próximo — Pagamento →'
              : flowStep === 'pagamento' ? 'Próximo — Detalhes →'
              : 'Revisar e Finalizar →';
            /* Nota(s) que ainda faltam preencher no passo atual — feedback claro. */
            const pendentes = activeFiliaisInCart.filter(f => !canProceedSubStep(f, flowStep as SubStep));
            return (
              <>
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-4">
                  ...
                  <button
                    ...
                    style={{ background: accent, color: 'var(--primary-foreground)', fontSize: 'var(--text-sm)', fontWeight: 'var(--font-weight-bold)', fontFamily: 'var(--font-red-hat-display)', outlineColor: accent }}>
                    {nextLabel}
                  </button>
                </div>
                {!stepCanProceed && (
                  <div className="flex items-start gap-2 -mt-1">
                    ...
                    <span className="block" style={{ fontSize: 'var(--text-xs)', color: 'var(--muted-foreground)', fontFamily: 'var(--font-red-hat-display)', lineHeight: '1.5' }}>
                      {flowStep === 'frete'
                        ? `Escolha o frete ${pendentes.length === 1 ? `da nota ${pendentes[0]}` : 'de cada nota'} para continuar.`
                        : `Selecione forma e condição de pagamento ${pendentes.length === 1 ? `da nota ${pendentes[0]}` : 'de cada nota'} para continuar.`}
                    </span>
                  </div>
                )}
              </>
        ...
        {/* Right column — sidebar combinado (resumo por nota + total geral) */}
        {renderCombinedSidebar()}
      </div>
    </div>
  );
}
