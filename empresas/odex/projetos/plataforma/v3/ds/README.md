...
> v0.41.0 · 2026-05-20 · Owner: Gabriel Felix Barbosa
...
- [stepper-input](./atoms/stepper-input.md) — `.ds-stepper-input` ⭐ Phase H.11 · 3 DUPES · qty stepper -/+ · 3 sizes × filled variant
- [tabs](./atoms/tabs.md) — `.ds-tabs` ⭐ Phase H.12 · 4 DUPES · tab bar horizontal · underline default × pill variant · count badges
- [kv-list](./atoms/kv-list.md) — `.ds-kv-list` ⭐ Phase H.13 · 3 DUPES · key-value grid · 4 variants (default/flush/dashed/stacked)
- [link-back](./atoms/link-back.md) — `.ds-link-back` ⭐ Phase H.15 · 5 DUPES · "← Voltar" pattern · text + outlined variants
- [card-selectable](./atoms/card-selectable.md) — `.ds-card-selectable` ⭐ Phase H.16 · 3 DUPES · radio card · is-selected state + radio indicator
- [alert](./atoms/alert.md) — `.ds-alert` ⭐ Phase H.17 · 4 DUPES · banner inline · 5 tones (info/success/warning/danger/neutral) × 3 sizes + dismiss
- [modal](./atoms/modal.md) — `.ds-modal` ⭐ Phase H.18 · 3 DUPES · centered dialog · 5 sizes (sm/md/lg/xl/full) + head/body/foot slots
- [form-grid](./atoms/form-grid.md) — `.ds-form-grid` ⭐ Phase H.19 · 4 DUPES · responsive grid · 4 col counts + asymmetric variants · mobile collapse
- (H.20) `.ds-tab-pill` standalone — adicionado em `ds/atoms/tabs.css` · 2 DUPES (ped-v2-pill, novas-tab-pill) · pill rounded com active filled blue
...
- [x] **H.11 · `.ds-stepper-input`** (3 DUPES · pdp-qty, qty-stepper, cart-qty) — quantity stepper -/+ · 3 sizes × filled variant
- [x] **H.12 · `.ds-tabs`** (4 DUPES · pdp-tabs, admin-tabs, dash-products-tabs, notif-drawer-tabs) — tab bar · underline default × pill variant · count badges
- [x] **H.13 · `.ds-kv-list`** (3 DUPES · pdp-meta-grid, pdp-price-breakdown, ped-resumo-kv) — key-value grid · 4 variants (default/flush/dashed/stacked)
- [x] **H.14 · `.ds-btn-on-brand`** (3 DUPES · hero-cta, dash-hero-cta, ajuda-cta) — adicionado em `ds/atoms/buttons.css` como variant: `.ds-btn-on-brand` (white solid sobre brand) + `.ds-btn-on-brand-ghost` (glass)
- [x] **H.15 · `.ds-link-back`** (5 DUPES · orc-back, ped-detail-back, pv-back, client-back-btn, auth-back) — "← Voltar" pattern · 3 sizes × outlined variant
- [x] **H.16 · `.ds-card-selectable`** (3 DUPES · mk-kit-card, nc-tipo-opt, ov-radio-row) — radio card · is-selected state + radio indicator + vertical layout
- [x] **H.17 · `.ds-alert`** (4 DUPES · orc-alert, mk-tax-alert, ckform-alert, admin-banner-alert) — banner inline · 5 tones × 3 sizes + dismiss
- [x] **H.18 · `.ds-modal`** (3 DUPES · .modal, .ped-resumo-modal, .help-modal) — centered dialog · 5 sizes + head/body/foot slots · role=dialog + aria-modal
- [x] **H.19 · `.ds-form-grid`** (4 DUPES · .ck-grid, .nc-grid, .ov-grid, .mk-grid) — responsive grid · 4 col counts + asymmetric variants + span helpers + mobile auto-collapse
- [x] **H.20 · `.ds-tab-pill`** (2 DUPES · ped-v2-pill, novas-tab-pill) — pill rounded standalone (não dentro de `.ds-tabs`) · active filled blue + count badge · adicionado em `ds/atoms/tabs.css`
- [x] **PHASE H ATOMS COMPLETE · 20/20 atoms criados** (audit dizia 23 · alguns foram absorvidos como variants em atoms existentes: btn-on-brand em buttons, tab-pill em tabs, ds-dot-sep em status-dot, ds-icon-btn-badge em icon-btn)
- [ ] **H.21+ · Markup migration** — substituir uses por DS classes em features (Phase futura · grande sweep)
...
| 0.41.0 | H.18-H.20 final batch · **PHASE H ATOMS COMPLETE** | 2026-05-20 | **3 final atoms criados · 20/20 done:** **H.18** `.ds-modal` (3 DUPES · .modal/.ped-resumo-modal/.help-modal) · centered dialog · 5 sizes (sm 420 / md 560 default / lg 720 / xl 960 / full · max 1200) + head/body/foot slots · role=dialog + aria-modal pattern · pair com `.ds-overlay` backdrop · `.ds-modal-hero-head` variant pra compor com `.ds-hero-gradient` (help-modal pattern) · **H.19** `.ds-form-grid` (4 DUPES · .ck-grid/.nc-grid/.ov-grid/.mk-grid) · custom prop `--ds-form-grid-cols` (1/2/3/4) + asymmetric variants (`.ds-form-grid-21` 2fr 1fr · `.ds-form-grid-31` · `.ds-form-grid-city` 1fr 100px) + span helpers (`.ds-form-grid-field-full` 1/-1 · `.ds-form-grid-field-half`) + auto-collapse < 600px · **H.20** `.ds-tab-pill` (2 DUPES · ped-v2-pill/novas-tab-pill) · pill rounded standalone (não dentro de `.ds-tabs`) · `.is-active` filled blue + `.ds-tab-pill-count` badge · adicionado em `ds/atoms/tabs.css` (não arquivo separado · variant). **Total Phase H: 20/20 atoms confirmados criados** (audit dizia 23 · 3 foram absorvidos como variants: `.ds-btn-on-brand` em buttons.css · `.ds-tab-pill` em tabs.css · `.ds-dot-sep` em status-dot.css · `.ds-icon-btn-badge` em icon-btn.css). Próximo: H.21+ markup migration. |
| 0.40.0 | H.11-H.17 batch | 2026-05-20 | **7 atoms criados em batch** · todos com .css + .md + barrel wired · pure semantic tokens · zero hardcoded rgba: **H.11** `.ds-stepper-input` (3 DUPES · 3 sizes × filled variant · custom prop sizing) · **H.12** `.ds-tabs` (4 DUPES · underline default × pill variant + `.ds-tab-count` badge + `.ds-tab-pane` content) · **H.13** `.ds-kv-list` (3 DUPES · key-value grid · 4 style variants · custom `--ds-kv-cols`) · **H.14** `.ds-btn-on-brand` + `-on-brand-ghost` (3 DUPES · ADDED como variants em `ds/atoms/buttons.css` · consume `--white`/`--blue-50`/`--color-glass-*`) · **H.15** `.ds-link-back` (5 DUPES · 3 sizes × outlined variant · hover slide arrow animation) · **H.16** `.ds-card-selectable` (3 DUPES · radio card · `.is-selected` + `[aria-checked]` state · radio indicator slot · vertical layout) · **H.17** `.ds-alert` (4 DUPES · 5 tones info/success/warning/danger/neutral · 3 sizes + dismiss button slot). Progress: 17/23 atoms done · 6 restantes (H.18-H.20 baixa-prioridade + markup migration H.21+). |
