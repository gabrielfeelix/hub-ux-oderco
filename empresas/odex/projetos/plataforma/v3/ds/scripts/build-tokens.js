...
const RULES = [
  ...
  { test: (p) => p.startsWith('overlay.'),         build: (p) => '--color-overlay-' + p.slice('overlay.'.length) },
  { test: (p) => p.startsWith('glass.'),           build: (p) => '--color-glass-' + p.slice('glass.'.length) },
  { test: (p) => p.startsWith('tint.'),            build: (p) => '--color-tint-' + p.slice('tint.'.length) },
...
const SECTION_ORDER = [
  ...
  { prefix: 'overlay.',        header: 'OVERLAY · backdrop tints (navy-based · brand consistente)' },
  { prefix: 'glass.',           header: 'GLASS · rgba whites sobre brand bg' },
  { prefix: 'tint.',            header: 'TINT · brand colors com alpha (hover/focus subtle bg)' },
