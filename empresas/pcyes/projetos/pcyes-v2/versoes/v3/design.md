...
| 2026-05-28 | **QuickAddButton + auditoria de CTAs hardcoded** — auditoria achou 5 quick-add CTAs hand-rolled fora do `CTAButton` (retangulares, peso medium, distintos do pill canônico) e 3 deles com sombra hardcoded `0 8px 20px -6px rgba(34,197,94,.55)` desviada do token `--shadow-buy-cta-sm`. Extraído `<QuickAddButton label />` em `section/` reproduzindo o markup idêntico + token de sombra; migrados os 3 triplicados (ProductsByTags, ProductCarousel, PopularGrid). Total primitivos: 13. Recriado `tokens.studio.json` (formato nativo Tokens Studio, faltava neste branch). **Pendente (precisa browser p/ validar pixel):** migrar os 2 CTAs maiores `Comprar` (NewReleasesSection, FeaturedProduct) e os forms de CheckoutPage/CartPage; tokenizar sombras repetidas (`0 6px 18px -4px rgba(225,6,0,.5)` ×5, hairline inset ×11, azul `#009ee3` ×2). |
| 2026-06-03 | **Orquestração do DS (Fases 1–7)** — (1) **CTAButton v2**: API limpa `variant`(cor: buy/preorder/brand) × `size`(sm/md/lg) + `block` + `as="link"` + forma pílula única; sombra deriva de cor+tamanho; todos os buy CTAs convergidos. (2) Primitivo **`<CarouselNavButton>`** (ProductShelf, FlashDeals, ProductCarousel). (3) Primitivo **`<ProductCard>`** em `components/` (`variant` shelf/grid + toggles rank/swatches/favorite/hoverMedia/emphasizeDiscount); migrados ProductShelf, FlashDeals, DealsHighlight; halo aplicado em ProductsPage + ProductCarousel. Drop do Dia fica como `ProductCardPremium` separado. (4) **Duas linguagens de hover formalizadas por tipo de superfície**: card de produto = **halo** (`.deal-card-img`), banner/categoria = **stroke** (`.stroke-hover-red` novo, reusa `--shadow-category-active`). Fix de causa raiz: `box-shadow`/`border` inline sobrepunham o `:hover` de classe — removido/ajustado em DropDoDia, Essentials, IntelligentDevices, InRealLife, PopularGrid, ProductPage (relacionados), ProfilePage (favoritos). (5) **Camada de tokens semânticos** `--surface-0..3/glass`, `--ink-strong/ink/muted/subtle`, `--edge-subtle/edge/strong` (dark real + light rascunho) em `theme.css` + `@theme inline` (utilitários `bg-surface-*`, `text-ink-*`, `border-edge-*`) + exportados em `tokens.json` (grupo `color.semantic`). (6) **Migração de cor faseada 6a–6f**: **zero `text-white` e zero `border-white` no projeto inteiro** (~31 arquivos); fundos de seção `#0a0a0a/#0e0e0e/#1f1c1c…` → `var(--surface-*)`. Preservados: overlays `bg-white`/`bg-black`, `text-black`, blocos brancos (`#fff`/`#f5f5f5`) e hexes de marca (pagamentos, dourado, glow). (7) Deletados 2 componentes órfãos (NewReleasesSection, FeaturedProduct). Gates: 31 erros TS pré-existentes mantidos em toda a sessão, builds verdes, cada fase commit atômico. **Pendente:** milestone Light Mode (keep-dark em Navbar/Announce + valores light são rascunho); forms Cart/Checkout/Profile → FieldInput; hex inline em estados dinâmicos (ternários ativos); extração de `ProductCardPremium`; base Modal/Drawer; sweep de borderRadius. |
